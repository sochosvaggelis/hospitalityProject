            ...
            <section className="relative overflow-hidden min-h-screen flex flex-col justify-center">
                ...
                <div className="relative max-w-7xl mx-auto px-4 sm:px-6 pt-20 pb-12 sm:pt-36 sm:pb-28 flex items-start min-h-screen">
                    <div className="max-w-4xl mx-auto text-center w-full">
                        <h1 className="animate-hero font-display text-3xl sm:text-5xl lg:text-6xl font-bold text-white leading-tight drop-shadow-sm">
                            {lang === 'el' ? 'Βρες δουλειά ως σερβιτόρος στα ελληνικά νησιά' : 'Find Server Jobs on Greek Islands'}
                        </h1>
                        <p className="animate-hero mt-4 sm:mt-5 text-lg sm:text-2xl font-bold leading-snug max-w-3xl mx-auto" style={{ animationDelay: '0.1s' }}>
                            <span style={{ color: '#5bc4e0' }}>{lang === 'el' ? 'Θες διακοπές και δουλειά; ' : 'Want a job that feels like vacation? '}</span>
                            <span className="text-white">{lang === 'el' ? 'Σε έχουμε.' : "We've got you."}</span>
                        </p>
                        ...
                        <div className="animate-hero mt-6 sm:mt-10 flex flex-col sm:flex-row gap-3 max-w-xl mx-auto p-2 rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 shadow-2xl" style={{ animationDelay: '0.3s' }}>
                            <div className="relative flex-1">
                                ...
                                <Input className="pl-10 h-11 sm:h-12 rounded-xl bg-white/95 border-0 text-base shadow-inner"
                        ...
                        <div className="animate-hero mt-4 sm:mt-6 flex items-center justify-center" style={{ animationDelay: '0.4s' }}>
                            <Link to="/jobs">
                                <Button variant="outline" className="rounded-xl gap-2 bg-white/10 text-white border-white/50 hover:bg-white/20 hover:text-white text-sm sm:text-base backdrop-blur-sm">
                        ...
                        {/* Stats */}
                        <div className="animate-hero mt-8 sm:mt-14 flex items-center justify-center gap-5 sm:gap-10" style={{ animationDelay: '0.5s' }}>
                            {[
                                { value: animatedJobs, label: lang === 'el' ? 'Αγγελίες' : 'Active jobs' },
                                { value: animatedIslands, label: lang === 'el' ? 'Νησιά' : 'Islands' },
                                { value: animatedCategories, label: lang === 'el' ? 'Κατηγορίες' : 'Categories' },
                            ].map((stat, i) => (
                                <div key={stat.label} className="contents">
                                    {i > 0 && <div className="w-px h-10 sm:h-14 bg-white/20" />}
                                    <div className="text-center min-w-[72px] sm:min-w-[110px]">
                                        {loading ? (
                                            <div className="h-9 sm:h-[60px] flex items-center justify-center">
                                                <div className="w-12 sm:w-16 h-7 sm:h-10 rounded-lg bg-white/15 animate-pulse" />
                                            </div>
                                        ) : (
                                            <div className="text-3xl sm:text-6xl font-black" style={{ color: '#38d4f5', textShadow: '0 0 20px rgba(56,212,245,0.5)' }}>{stat.value}</div>
                                        )}
                                        <div className="text-xs sm:text-lg font-semibold mt-0.5 sm:mt-1 text-white/80">{stat.label}</div>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5 sm:gap-4">
                        {loading && Array.from({ length: 10 }).map((_, i) => (
                            <div key={i} className="rounded-xl sm:rounded-2xl aspect-[3/4] animate-pulse flex flex-col items-center justify-between p-2.5 sm:p-4"
                                style={{ background: 'hsl(40 50% 97%)', boxShadow: '0 2px 12px -2px rgba(194,160,100,0.2)' }}>
                                <div className="h-3.5 w-2/3 rounded bg-muted" />
                                <div className="w-3/4 h-1/2 rounded-2xl bg-muted/70" />
